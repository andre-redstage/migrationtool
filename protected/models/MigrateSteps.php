...
class MigrateSteps extends MigrateStepsPeer
{
    ...
    public static function checkStep($currentStepIndex = null){
        ...
        if($currentStepIndex == 5)
        {
            ...
            $criteria->select = "t.sorder";
            $criteria->order = "t.sorder ASC";
            $criteria->condition = "t.sorder < 4 AND status = ". self::STATUS_NOT_DONE;
            $step = MigrateSteps::model()->find($criteria);

            if ($step){
                $result['allowed'] = false;
                $result['back_step'] = "step3";
            }
        }
        ...
        if($currentStepIndex == 2)
        {
            ...
            $criteria->select = "t.sorder";
            $criteria->order = "t.sorder ASC";
            $criteria->condition = "t.sorder = 1 AND status = ". self::STATUS_NOT_DONE;
            $step = MigrateSteps::model()->find($criteria);

            if ($step){
                $result['allowed'] = false;
                $result['back_step'] = "step1";
            }
        }
        ...
        if($currentStepIndex > 2)
        {
            ...
            $criteria->select = "t.sorder";
            $criteria->order = "t.sorder ASC";
            $criteria->condition = "t.sorder = 2 AND status = ". self::STATUS_NOT_DONE;
            $step = MigrateSteps::model()->find($criteria);
            if ($step){
                $result['allowed'] = false;
                $result['back_step'] = "step2";
            }
        }
